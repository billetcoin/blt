...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BLT);
    ...
    if (settings.contains("nAnonymizeBilletcoinAmount") || reset)
        SoftSetArg("-anonymizebilletcoinamount", settings.value("nAnonymizeBilletcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in billetcoin.cpp during shotdown to also remove the window positions
