...
#include "qt/billetcoin/send.h"
#include "qt/billetcoin/forms/ui_send.h"
#include "qt/billetcoin/addnewcontactdialog.h"
#include "qt/billetcoin/qtutils.h"
#include "qt/billetcoin/sendchangeaddressdialog.h"
#include "qt/billetcoin/optionbutton.h"
#include "qt/billetcoin/sendconfirmdialog.h"
#include "qt/billetcoin/myaddressrow.h"
#include "qt/billetcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(BilletcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (BLT)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Billetcoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BLT");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_BLT : AskPassphraseDialog::Context::Send_zBLT, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zBLT is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zBLT selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zBLT transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZBLT_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zBLT require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBLT transaction
        ...
        emit message("zBLT transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isBLT){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zBLT in your balance to select."));
        }
    }
}
...
void SendWidget::onBLTSelected(bool _isBLT){
    isBLT = _isBLT;
    setCssProperty(coinIcon, _isBLT ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
