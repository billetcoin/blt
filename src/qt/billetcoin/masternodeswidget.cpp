...
#include "qt/billetcoin/forms/ui_masternodeswidget.h"
#include "qt/billetcoin/qtutils.h"
#include "qt/billetcoin/mnrow.h"
#include "qt/billetcoin/mninfodialog.h"

#include "qt/billetcoin/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(BilletcoinGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i BLT requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 BLT required."));
            return;
        }
        showHideOp(true);
