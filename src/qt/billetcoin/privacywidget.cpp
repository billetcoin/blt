...
#include "qt/billetcoin/privacywidget.h"
#include "qt/billetcoin/forms/ui_privacywidget.h"
#include "qt/billetcoin/qtutils.h"
#include "guiutil.h"
#include "qt/billetcoin/denomgenerationdialog.h"
#include "qt/billetcoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(BilletcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zBLT anonymizes your BLT by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zBLT or convert back to BLT"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 BLT ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zBLT");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zBLT");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zBLT");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zBLT");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zBLT");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zBLT");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zBLT");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zBLT");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zBLT Movements"));
    ...
    // Only Convert to BLT enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzBLT->setTitleClassAndText("btn-title-grey", "Total 0 zBLT");
    ui->btnTotalzBLT->setSubTitleClassAndText("text-subtitle", "Show denominations of zBLT owned.");
    ui->btnTotalzBLT->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BLT outputs to mint into zBLT.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zBLT");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzBLT, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zBLT");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BLT to mint into zBLT"));
    }else{
        btnText = tr("Convert back to BLT");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zBLT to convert back into BLT"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzBLT->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzBLT->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBLT is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zBLT, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zBLT").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zBLT minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zBLT converted back to BLT"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any BLT to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBLT </b>";
    ...
    ui->btnTotalzBLT->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
