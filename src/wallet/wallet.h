...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BLT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBLT send
enum ZerocoinSpendStatus {
    ZBLT_SPEND_OKAY = 0,                            // No error
    ZBLT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBLT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBLT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBLT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBLT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBLT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBLT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBLT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBLT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBLT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBLT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBLT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBLT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBLT_SPENT_USED_ZBLT = 14,                      // Coin has already been spend
    ZBLT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBLT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZBLTOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzBLTTracker> zpivTracker;
    ...
    void setZWallet(CzBLTWallet* zwallet);
    CzBLTWallet* getZWallet();
    ...
    /// Get 10000 BLT output and keys which can be used for the Masternode
    ...
    /** zBLT reset */
    boost::signals2::signal<void()> NotifyzBLTReset;
