...
public:
    ...
    bool WriteZBLTSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZBLTSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZBLTSeed_deprecated(uint256& seed);
    bool EraseZBLTSeed();
    bool EraseZBLTSeed_deprecated();

    bool WriteZBLTCount(const uint32_t& nCount);
    bool ReadZBLTCount(uint32_t& nCount);
