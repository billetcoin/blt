...
CzBLTWallet::CzBLTWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBLTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBLTSeed_deprecated()) {
                    LogPrintf("%s: Updated zBLT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzBLTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBLTCount(nCountLastUsed);
    else if (!walletdb.ReadZBLTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBLTWallet::Lock()
{
    seedMaster = 0;
}

void CzBLTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBLTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBLT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBLTWallet::LoadMintPoolFromDB()
{
...
void CzBLTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBLTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBLTWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzBLTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBLT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzBLTWallet::SeedToZBLT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBLTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBLTWallet::GenerateDeterministicZBLT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBLTWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBLTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBLTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
