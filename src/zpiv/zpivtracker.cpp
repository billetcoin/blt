// Copyright (c) 2018-2020 The Billetcoin developers
...
CzBLTTracker::CzBLTTracker(std::string strWalletFile)
{
...
CzBLTTracker::~CzBLTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBLTTracker::Init()
{
...
bool CzBLTTracker::Archive(CMintMeta& meta)
{
...
bool CzBLTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBLTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBLTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBLTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBLTTracker::GetSerialHashes()
{
...
CAmount CzBLTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBLTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBLTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBLTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBLTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBLTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBLTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBLTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBLTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBLTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBLTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBLTWallet* zBLTWallet)
{
    bool iszBLTWalletInitialized = (NULL != zBLTWallet);
    CMintMeta meta;
    ...
    if (! iszBLTWalletInitialized)
        zBLTWallet = new CzBLTWallet(strWalletFile);
    meta.isSeedCorrect = zBLTWallet->CheckSeed(dMint);
    if (! iszBLTWalletInitialized)
        delete zBLTWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBLTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBLTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBLTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBLTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBLTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBLTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBLTWallet* zBLTWallet = new CzBLTWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBLTWallet);
        }
        delete zBLTWallet;
