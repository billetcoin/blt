...
*Setup instructions for a gitian build of Billetcoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Billetcoin
...
to the Billetcoin GitHub release page.
...
- [Building Billetcoin Core](#building-billetcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/billetcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/billetcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/billetcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/billetcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/billetcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/billetcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/billetcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@billetcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/billetcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/billetcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/billetcoin-osx-*-build.assert
```
